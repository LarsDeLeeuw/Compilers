...
class SemanticAnalysis(ASTVisitor):
    ...
    def visitArraySubscriptExprNode(self, node):
        if not node.array_child.parseType()[1]:
            print(bcolors.BOLD + "{}:{}:{}: ".format(self.inputfilepath, node.index_child.line, node.index_child.column) + bcolors.FAIL + "error: " + bcolors.RESET 
            + "subscripted value is not an array")
            self.buffer["errors"].append("{}:{}:{}: ".format(self.inputfilepath, node.index_child.line, node.index_child.column) + bcolors.FAIL + "error: " + bcolors.RESET 
            + "subscripted value is not an array")

        if node.index_child.type != "int":
            print("{}:{}:{}: ".format(self.inputfilepath, node.index_child.line, node.index_child.column) + bcolors.FAIL + "error: " + bcolors.RESET 
